...
class DefaultSmaugClient(requestFactory: HttpRequestFactory,
                         connectTimeout: Option[Duration] = None,
                         readTimeout: Option[Duration] = None,
                         numberOfRetries: Int = 0,
                         endpointUrl: String = Endpoints.production) extends SmaugClient {
  ...
  override def formUrl(params: Option[String] = None): Try[URL] = {
    Try {
      ...
      Option(getAndExtractLocationHeader(resource)) match {
        ...
        case None => throw SmaugInternalException("Form endpoint did not return location header")
      }
    }
  }
  ...
  override def tokenize(card: CreditCard, tenantId: String): Try[CreditCardToken] = {
    Try {
      ...
      case Failure(e) => Failure(new SmaugInternalException(e.getMessage, e))
    }
  }
